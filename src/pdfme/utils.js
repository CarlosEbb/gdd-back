...
export function agregarImageSandbox(templateJson, width = null, height = null, base64Image = basetestvertical) {
  ...
  let marginLeft = 0, marginTop = 0, marginRight = 0, marginBottom = 0;
  ...
  nuevoTemplate.schemas = nuevoTemplate.schemas.map((schemaArray, index) => {
    ...
    const yaExisteSandbox = nuevoSchemaArray.some(field => 
      field.name && field.name.startsWith('TestTestLock')
    );
  ...
  console.log(`🖼️ Sandbox image centrada: área disponible ${availableWidth}x${availableHeight}, imagen ${imageWidth}x${imageHeight} en posición (${imageX},${imageY})`);
  return nuevoTemplate;
}